...
import com.sugood.ijplugin.tt_miniprogram.utils.contentRange
import com.sugood.ijplugin.tt_miniprogram.utils.findChildOfType

/**
 * 检查ttss的import是否有效
 */
class TTSSInvalidImportInspection : LocalInspectionTool() {

    companion object {
        const val QUICK_FIX_FAMILY_NAME = "TTSS导入语句"
    }
            ...
            override fun visitCssImport(wxssImport: CssImport) {
                ...
                if (stringText.isBlank()) {
                    holder.registerProblem(
                            string, TextRange.allOf(string.text), this@TTSSInvalidImportInspection.displayName
                    )
                    return
                }
                ...
                if (references.isEmpty()) {
                    holder.registerProblem(
                            string, string.contentRange(), this@TTSSInvalidImportInspection.displayName
                    )
                } else {
