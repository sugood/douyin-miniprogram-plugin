...
import com.sugood.ijplugin.tt_miniprogram.lang.expr.TtmlJsLanguage
import com.sugood.ijplugin.tt_miniprogram.lang.ttml.TtmlJSInjector.Companion.DOUBLE_BRACE_REGEX
import com.sugood.ijplugin.tt_miniprogram.lang.ttml.utils.isEventHandler
import com.sugood.ijplugin.tt_miniprogram.lang.ttml.utils.valueTextRangeInSelf
import com.sugood.ijplugin.tt_miniprogram.utils.toTextRange

class TtmlJSInjector : MultiHostInjector {
    ...
    override fun getLanguagesToInject(multiHostRegistrar: MultiHostRegistrar, psiElement: PsiElement) {
        if (psiElement.language is TTMLLanguage) {
            when (psiElement) {
                is XmlTextImpl -> {
    ...
    private fun injectInAttributeValue(
        ...
        if (tagName == "sjs" || tagName == "include" || tagName == "import"
                || (tagName == "template" && attributeName == "name")
                || attributeName == "tt:for-item" || attributeName == "tt:key" || attributeName == "tt:for-index") {
            // wxs等特殊标签 标签的属性不支持 {{}} 语法
            // tt:for 相关的一些辅助属性不支持 {{}} 语法
            return
        }
        if (attribute.isEventHandler() && !DOUBLE_BRACE_REGEX.matches(psiElement.text)) {
            // 此属性是事件
            // 并且属性值中没有双括号
            multiHostRegistrar.startInjecting(TtmlJsLanguage.INSTANCE)
